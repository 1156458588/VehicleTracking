...
# main_2
def getCars(data, threshold, distance, relHeight, channel, originalData):

    retCars = []
    data = np.array(data)
    peaks, _ = scipy.signal.find_peaks(data, height=threshold, distance=distance)
    prominence2 = peak_prominences(data, peaks)[0]
    print('peaks = ', peaks)
    results_half = peak_widths(data, peaks, rel_height=relHeight)
    for i in range(len(peaks)):
        carFeature = np.average(np.abs(originalData[:, peaks[i]]))
        zeroCrossRate = ZeroCrossingRate(originalData[:, peaks[i]])
        print('最大值是：', np.average(np.abs(originalData[:, peaks[i]])))
        start = int(round(results_half[2][i]))
        end = int(round(results_half[3][i]))
        prominence = prominence2[i]
        ...
        #     carType = 1
        # 底噪
        # if  zeroCrossRate > 0.09:
        #     continue
        # 大车
        if carFeature > 1:
            carType = 1
        # 底噪（很重要）(200点)
        # if prominence < 8 or (length > 6 and prominence < 10):
        #     continue
        # 500点
        if prominence < 20 or (length > 6 and prominence < 25):
            continue
        # if channel == 1:
        #     print('asd定位点：', peaks[i], ", 突起度=", prominence, '通道：', channel, '范围:', (end - start + 1))
        retCars.append(car(scopes=[start, end], carType=carType,
                           position=peaks[i],
                           peakValue=carFeature, channel=channel, length=length))
    return retCars
...
#             carType = 1
#         # 底噪
#         if (prominence < 20 and length > 3) or prominence < 10 or length > 6:
#             continue
#         if channel == 1:
#             print('asd定位点：', peaks[i], ", 突起度=", prominence, '通道：', channel, '范围:', (end - start + 1))
#         retCars.append(car(scopes=[start, end], carType=carType,
