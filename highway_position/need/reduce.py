...
def get_clean_data(data_frame, response_weight=True):
    ...
    for channel, channel_data in enumerate(data_frame):
        ...
        data_frame_aline.append(channel_data[:, sta:end])
    data_frame_aline = np.array(data_frame_aline)

    data_frame_abs = np.abs(data_frame_aline)
